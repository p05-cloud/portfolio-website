document.addEventListener('DOMContentLoaded', function () {
    // Detect touch devices for performance optimization
    const isTouchDevice = 'ontouchstart' in window || navigator.maxTouchPoints > 0 || navigator.msMaxTouchPoints > 0;
    const prefersReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

    // Enhanced Loading Animation (skip on touch devices for better performance)
    ...
    // Typing Animation for Hero Text (skip on touch devices)
    if (!isTouchDevice && !prefersReducedMotion) {
    ...
    // Floating Particles Background (skip on touch devices)
    if (!isTouchDevice && !prefersReducedMotion) {
    ...
    // Parallax Effect for Background Elements (skip on touch devices)
    if (!isTouchDevice && !prefersReducedMotion) {
        window.addEventListener('scroll', () => {
